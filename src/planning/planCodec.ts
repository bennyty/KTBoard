...
type CompactPlan = [string, string, string, CompactSlide[]]
...
export function encodePlan(plan: Plan): string {
  const compact: CompactPlan = [plan.name, plan.mapId, plan.dropZoneId, plan.slides.map(encodeSlide)]
...
export function decodePlan(encoded: string): Plan | null {
  ...
  if (!Array.isArray(parsed) || !isStr(parsed[0]) || !isStr(parsed[1]) || !isStr(parsed[2]) || !Array.isArray(parsed[3])) {
    return null
  }
  const slides: Slide[] = []
  for (const raw of parsed[3]) {
    const s = decodeSlide(raw)
    if (!s) return null
    slides.push(s)
  }
  if (slides.length === 0) return null
  return { name: parsed[0], mapId: parsed[1], dropZoneId: parsed[2], slides }
}
